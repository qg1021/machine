...
<head>
	...
	<title>东方农机商城(www.machine1688.com)-${product.name}-评价</title>
	...
	function onDisplay(){
		...
		if(confirm("确认显示？")){
			$("#mainForm").attr("action","post!batchDisplay.action?pid=${pid}").submit();
		}
	}
	function onDisable(){
		var  isChecked=false;
		...
		if(confirm("确认屏蔽？")){
			$("#mainForm").attr("action","post!batchDisable.action?pid=${pid}").submit();
		}
	}
	</script>
</head>
<body> 
	<%@include file="/common/adminheader.jsp" %>
		...
		<div id="content"> 
    		...
    		<div class="mainpart">
    			<div class="title">
      				<h2 class="ll">${product.name}-评价</h2>
    			</div>
    			<div class="detailbox">
					<div id="message" style="line-height:20px;">
						<s:actionmessage theme="custom" cssClass="tipbox"/>
					</div>
	    			<form id="mainForm" action="post.action?pid=${pid}" method="post">
		        			...
		        			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebg font333 ll collapse">
			    				...
			    				<s:iterator value="page.result" status="st">
								   <tr>
								        ...
								        <td>${remark}</td>
								        <td>
								        	<a href="post!input.action?id=${id}&pid=${productid}">查看</a>
								        	<s:if test="status">
								        		<a href="post!batchDisplay.action?ids=${id}&pid=${productid}">显示</a>
								        	</s:if>
								        	<s:else>
								        		<a href="post!batchDisable.action?ids=${id}&pid=${productid}">屏蔽</a>
								        	</s:else>
								        </td>
								    </tr>
							    </s:iterator>
