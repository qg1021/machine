...
<head>
	...
	function onBatchDelete(){
		...
		if(confirm("确认删除？")){
			$("#mainForm").attr("action","links!batchDelete.action?mtype=${mtype}").submit();
		}
	}
	function onPublish(){
		var  isChecked=false;
		...
		if(confirm("确认发布？")){
			$("#mainForm").attr("action","links!publish.action?mtype=${mtype}").submit();
		}
	}
	function onCancel(){
		var  isChecked=false;
		...
		if(confirm("确认取消发布？")){
			$("#mainForm").attr("action","links!cancelPublish.action?mtype=${mtype}").submit();
		}
	}
	...
	<%@include file="/common/adminheader.jsp" %>
		...
		<div id="content"> 
    		...
    		<div class="mainpart">
    			...
    			<form id="mainForm" action="links.action?mtype=${mtype}" method="post">
					...
					<p class="ll mt15">
	    				<button id="btnCreate" type="button" onclick="window.location.href='links!input.action?mtype=${mtype}'">新增</button>
	        			...
	        			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebg font333 ll collapse">
		    				...
		    				<s:iterator value="page.result" status="st">
							   <tr>
							        ...
							        <td>${statusName}</td>
							        <td>
							        	<a href="links!input.action?id=${id}&mtype=${mtype}">修改</a>&nbsp;&nbsp;
							        	<a href="javaScript:delRecord('links!delete.action?id=${id}&mtype=${mtype}');">删除</a>
							        </td>
							    </tr>
						    </s:iterator>
