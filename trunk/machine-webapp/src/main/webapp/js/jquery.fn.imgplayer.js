 ...
 */
 (function($) {
    $.fn.player = function(settings) {
        ...
        defaultCSS.liFoucs = $.extend({},
        defaultCSS.li, defaultCSS.liFoucs);
        defaultCSS.liBlur = $.extend({},
        defaultCSS.li, defaultCSS.liBlur);

        var options = $.extend({
            showTitle: true,
            // 标题显示开关，true表示显示标题，默认为true
            showIndex: true,
            // 标题显示开关，true表示显示序号，默认为true
            width: '',
            // 播放器div容器的宽度
            height: '',
            // 播放器div容器的高度
            time: 2000,
            // 图片播放时间（单张图片播放至切换到下一张图片所要的时间）,单位：毫秒
            onStart: function() {},
            // 开始播放时执行的函数
            onStop: function() {},
            // 停止播放时执行的函数
        ...
        defaultCSS.size, settings, {
            divCSS: $.extend(defaultCSS.div, settings.divCSS || {},
            defaultCSS.size, {
                width: settings.width,
                height: settings.height
            }),
            // 用户自定义容器样式
            imgCSS: $.extend(defaultCSS.img, settings.imgCSS || {},
            defaultCSS.size, {
                width: settings.width,
                height: settings.height
            }),
            // 用户自定义图片样式
            ulCSS: $.extend(defaultCSS.ul, settings.ulCSS || {}),
            // 用户自定义图片序号容器UL样式
            liCSS: $.extend(defaultCSS.li, settings.liCSS || {}),
            // 用户自定义图片序号容器LI样式
            titleCSS: $.extend(defaultCSS.title, settings.titleCSS || {}),
            // 用户自定义标题样式
            focusIdxCSS: $.extend(defaultCSS.liFoucs, settings.focusIdxCSS || {}),
            // 图片序号focus样式
            blurIdxCSS: $.extend(defaultCSS.liBlur, settings.blurIdxCss || {})
            // 图片序号blur样式
        ...
        },
        {
            animate: $.extend({
                duration: 500,
                // 图片播放动画执行时间,建议此项小于time值的1/3,单位：毫秒
                method: '',
                // 动画效果，总共32种，默认随机
                direction: '',
                // 动画方向，总共8种，默认随机，zoomshow和zoomhide不支持随机。
                zoomRatio: 5
                // 采用zoomin或zoomout时的最大放大级别，默认5倍，大于等于零的整数
        ...
        var $container = this.hide().css(options.divCSS);
        // 获得图片播放器容器
        var $images = $container.find("img").hide().css(options.imgCSS);
        // 获得图片数组
        ...
        $container.initAnimate = function(animate) {
            ...
            switch (cur_animate.direction) {
                ...
                options.onShow = function($img) {
                    return $img.fadeIn(cur_animate.duration);
                };
                options.onHide = function($img) {
                    return $img.fadeOut(cur_animate.duration);
                };
                break;
                case 'left':
                // 水平向左
                options.onShow = function($img) {
                    ...
                    cur_animate.duration, cur_animate.method);
                ...
                options.onShow = function($img) {
                    ...
                    function() {
                        ...
                        // 动画执行结束后置为中间层，采用当前图片作为下一次动画执行背景
                ...
                return this.initAnimate($.extend({},
                options.animate, {
