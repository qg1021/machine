...
function setCookieMills(a, b, d) {
    var c = new Date();
    c.setTime(c.getTime() + d);
    document.cookie = a + "=" + escape(b) + ";expires=" + c.toGMTString() + ";path=/;domain=360buy.com"
...
document.onclick = function funClick(c) {
    c = c || event;
    var q = document;
    var u = c.srcElement || c.target;
    var h = $(u).attr("clstag");
    var l = c.clientX + (q.body.scrollLeft || q.documentElement.scrollLeft),
    i = c.clientY + (q.body.scrollTop || q.documentElement.scrollTop),
    r = q.body.clientWidth,
    o = q.body.clientHeight;
    var n = "";
    while (!h) {
        u = u.parentNode;
        if (!u) {
    ...
    var f = /(sale)|(mall)|(jmall).360buy.com/;
    if (f.test(g)) {
        log("d", "c", n || "-", l + "x" + i, r + "x" + o)
...
var SucInfoMethod = {
    Init: function() {
        ...
        var b = SucInfo_OrderDetail.split(",");
...
function loginfo(k, j, a, e, h) {
    var g = k.split(",");
    var c = SucInfo_OrderId,
    f = SucInfo_OrderType,
    b = SucInfo_OrderDetail;
...
function isChecked(a) {
    if (typeof(SucInfo_OrderId) == "undefined" && typeof(SucInfo_OrderType) == "undefined" && typeof(SucInfo_OrderDetail) == "undefined") {
        return false
...
function funLoad() {
    var d = "Club,ThirdRec,AttRec,OCRec,SORec,EBRec,BookSpecial,BookTrack,BookHis,Coupon,GlobalTrack,GlobalHis,History,historyreco_s,historyreco_c";
    var c = this.location.pathname.toLowerCase();
    var b = getCookie("pin");
    if (b != null && b.length > 0) {
        setCookieMills("rpin", b, 259200000)

    }
    if (isChecked(c)) {
        SucInfoMethod.Init();
        var g = getCookie("_distM");
        if (g != null && g == SucInfo_OrderId) {
            return true
        ...
        var n = e.split(":");
        simpleMold(n, "R", "reWids", "4");
        var m = ["p000", "p100", "np000", "np100"];
        for (var h = 0; h < m.length; h++) {
            var j = getCookie(m[h]);
            if (j != null && j != "") {
                log("HomePageOrder", m[h])
        ...
        var f = d.split(",");
        complexMold(f, "R", "reWids", "4");
        var k = ["v", "TrackRec", "TrackHis", "Zd", "Tc", "g", "s", "Book", "BookSpecial", "BookTrack", "BookHis", "GlobalTrack", "GlobalHis", "History", "Hiss", "Hisc"];
        complexMold(k, "o", "rod", "d", true);
        var a = ["_rdCube"];
        complexMold(a, "Cube", "", "4");
        var l = ["SEO"];
        simpleMold(l, "S", "seWids", "4");
                                        ...
                                        if (b == "/profile") {
                                            ...
                                            if (b.indexOf("/search/", 0) == 0) {
                                                ...
                                                if (b == "/favorite") {
                                                    ...
                                                    if (b.indexOf("/follow/", 0) == 0) {
                                                        log("t", "follow")

                                                    } else {
                                                        if (b.indexOf("/fans/", 0) == 0) {
                                                            log("t", "fans")

                                                        } else {
                                                            if (b.indexOf("/blacklist/", 0) == 0) {
                                                                log("t", "blacklist")
...
function getHistory() {
    ...
    var c = getCookie("_ghis");
    if (c == null && b != null) {
        var a = "http://gh.360buy.com/BuyHistory.aspx?mid=" + encodeURIComponent(b);
        $.ajax({
            url: a,
            type: "GET",
            dataType: "jsonp",
    ...
    Ttracker = {
        ...
        trace: function() {
            ...
            var thref = $(crumb[crumbIndex]).attr("href");
            if (thref == "" || thref == undefined) {
                return

            }
            var tb = thref.lastIndexOf("/"),
            te = thref.lastIndexOf("."),
            tts = thref.substring(tb + 1, te);
            var ttss = tts.split("-");
            var tt = ttss.length == 3 ? ttss[2] : ttss[0];
            var ttx = Number(tt);
            if (ttx.toString() == "NaN" || ttx == 0) {
                return

            }
            var wid = $("#name").attr("PshowSkuid") || (pageConfig ? (pageConfig.product ? pageConfig.product.skuid: 0) : 0);
            this.view(ttx, wid);
            this.viewtypewid(tt, wid)

        },
        viewtypewid: function(t, s) {
            var nview = new ThirdType(t, s);
            var sortmap = new SortedHashMap(this.IComparer, this.IGetKey);
            ...
            Ttracker.util.Wv("typewid", sortmap.GetJson(), 63072000000)

        },
        viewhisotry: function(t, s, cname) {
            var nview = new viewrecord(t, s);
            ...
            Ttracker.util.Wv(cname, bookmap.GetJson(), 63072000000)

        },
        viewrate: function(t, s, cname) {
            var nview = new viewsort(t, s);
            var sitesortmap = new SortedHashMap(this.IComparer, this.IGetKey);
            var maps = Ttracker.util.Vv(cname);
            if (maps) {
                try {
                    var maplist = eval("(" + maps + ")");
                    if (maplist.length > 0 && maplist[0].d != undefined) {
                        Ttracker.util.Wv(cname, "", -63072000000)
            ...
            if (!sitesortmap.ContainsKey(t)) {
                sitesortmap.Add(t, nview)

            } else {
                var curtt = sitesortmap.Get(t);
                curtt.s = s === 0 ? curtt.s: s;
                curtt.v += 2
            ...
            Ttracker.util.Wv(cname, sitesortmap.GetJson(), 63072000000)

        },
        view: function(t, s) {
            s = Number(s);
            if (this.isbook(s)) {
                this.viewhisotry(t, s, "bview");
                this.viewrate(t, s, "btw")

            }
            s && this.viewhisotry(t, s, "aview");
            this.viewrate(t, s, "atw")
    ...
    r.util = {
        ...
        Vv: function(y) {
            for (var g = [], t = al.cookie[J](";"), l = RegExp("^\\s*" + y + "=\\s*(.*?)\\s*$"), s = 0; s < t[Q]; s++) {
                var p = t[s]["match"](l);
                p && g[N](p[1])
    ...
    var aK = function() {
        ...
        this.set(ai, "UA-J2011-1");
        this.set(I, ".360buy.com");
        this.set(af, k());
        this.set(d, Math.round((new Date).getTime() / 1000));
        this.set(aG, 63072000000);
        this.set(V, 15768000000);
        this.set(am, 1800000);
        this.set(aL, S());
        var g = Z();
        this.set(Y, g.name);
        this.set(T, g.version);
        this.set(B, G());
        var l = aF();
        this.set(x, l.D);
        this.set(M, l.C);
        this.set(ay, l.language);
        this.set(n, l.javaEnabled);
        this.set(A, l.characterSet);
        this.set(aH, ak);
        this.set(aP, new Date().getTime())
    ...
    aF = function() {
        ...
        l.characterSet = al.characterSet || al.charset || "-";
        return l
    ...
    S = function() {
        ...
        t;
        y = "ShockwaveFlash";
        if ((l = (l = window.navigator) ? l.plugins: R) && l[Q] > 0) {
            for (C = 0; C < l[Q] && !t; C++) {
                y = l[C],
                y.name[w]("Shockwave Flash") > -1 && (t = y.description[J]("Shockwave Flash ")[1])
            ...
            y = y + "." + y;
            try {
                C = new ActiveXObject(y + ".7"),
                t = C.GetVariable("$version")

            } catch(s) {}
            if (!t) {
                try {
                    C = new ActiveXObject(y + ".6"),
                    t = "WIN 6,0,21,0",
    ...
    aj = function() {
        var p = aF();
        for (var l = p, g = ab.navigator, l = g.appName + g.version + l.language + g.platform + g.userAgent + l.javaEnabled + l.D + l.C + (al.cookie ? al.cookie: "") + (al.referrer ? al.referrer: ""), g = l.length, s = ab.history.length; s > 0;) {
            l += s--^g++
    ...
    aB = function(l) {
        l.set(ad, al.location.hostname);
        l.set(ah, al.title);
        l.set(aw, al.location.pathname);
        l.set(av, al.referrer);
        l.set(aC, al.location.href);
        var C = r.util.Vv("__jda"),
        s = C[Q] > 0 ? C[0][J](".") : null;
        l.set(ac, s ? s[1] : X());
        l.set(i, s ? s[2] : l.get(d));
        l.set(a, s ? s[3] : l.get(d));
        l.set(aJ, s ? s[4] : l.get(d));
        l.set(au, s ? s[5] : 1);
        var t = r.util.Vv("__jdv"),
        g = t[Q] > 0 ? t[0][J]("|") : null;
        l.set(e, g ? g[1] : "direct");
        l.set(ar, g ? g[2] : "-");
        l.set(c, g ? g[3] : "none");
        l.set(an, g ? g[4] : "-");
        var y = r.util.Vv("__jdb"),
        p = y[Q] > 0 ? y[0][J](".") : null;
        l.set(P, p ? p[1] : 0);
        l.set(b, aD() || "-");
        return ! 0
    ...
    aA = function(aX) {
        var s = al.location.href,
        C = al.referrer,
        aU = al.domain,
        y = r.util.getParameter(s, "utm_source"),
        t = [],
        W = aX.get(e),
        O = aX.get(ar),
        L = aX.get(c),
        D = aX.get(an),
        g = r.util.Vv("__jdb")[Q] == 0;
        if (y) {
            var l = r.util.getParameter(s, "utm_campaign"),
            aW = r.util.getParameter(s, "utm_medium"),
            ...
            aV = false;
            if (p && p[w](aU) < 0) {
                for (var aQ = aX.get(aH), aR = 0; aR < aQ.length; aR++) {
                    var aT = aQ[aR][J](":");
                    if (p[w](aT[0][H]()) > -1 && C[w]((aT[1] + "=")[H]()) > -1) {
                        var aS = unescape(aT[1][H]()),
                        K = r.util.getParameter(C, aS);
                        t[N](aT[0]),
                        t[N]("-"),
                        t[N]("organic"),
                        t[N](az(K) || "not set");
        ...
        if ((t[Q] > 0 && t[2] !== "referral" && (t[0] != W || t[1] != O || t[2] != L || t[3] != D)) || (g && t[Q] > 0 && t[2] === "referral")) {
            aX.set(e, t[0] || "direct");
            aX.set(ar, t[1] || "-");
            aX.set(c, t[2] || "none");
            aX.set(an, t[3] || "-");
    ...
    u = function(g) {
        return [g.get(af), g.get(ac) || "-", g.get(i) || "-", g.get(a) || "-", g.get(aJ) || "-", g.get(au) || 1][F](".")

    },
    f = function(g) {
        return [g.get(af), g.get(P) || 1, g.get(ac) + "|" + g.get(au) || 1, g.get(aJ) || g.get(d)][F](".")

    },
    z = function(g) {
        return [g.get(af), g.get(e) || al.domain, g.get(ar) || "(direct)", g.get(c) || "direct", g.get(an) || "-"][F]("|")
    ...
    o = function(l, g) {
        g.add("jdac", l.get(ai)),
        g.add("jduid", l.get(ac)),
        g.add("jdsid", l.get(ac) + "|" + l.get(au)),
        g.add("jdje", l.get(n)),
        g.add("jdsc", l.get(M)),
        g.add("jdsr", l.get(x)),
        g.add("jdul", l.get(ay)),
        g.add("jdcs", l.get(A)),
        g.add("jddt", az(l.get(ah)) || "-"),
        g.add("jdmr", az(l.get(av))),
        g.add("jdhn", l.get(ad) || "-"),
        g.add("jdfl", l.get(aL)),
        g.add("jdos", l.get(B)),
        g.add("jdbr", l.get(Y)),
        ...
        g.add("jdzb", l.get(au)),
        g.add("jdcb", l.get(P)),
        g.add("jdusc", l.get(e) || "direct"),
        g.add("jducp", l.get(ar) || "-"),
        g.add("jdumd", l.get(c) || "-"),
        g.add("jduct", l.get(an) || "-"),
        g.add("jdlt", typeof jdpts != "object" ? 0: jdpts._st == undefined ? 0: l.get(aP) - jdpts._st),
        g.add("jdtad", l.get(b))
    ...
    aN = function(l, g, p, s) {
        ...
        g.add("jdyb", "-"),
        g.add("jdzb", l.get(au)),
        g.add("jdcb", s ? ((l.get(P) * 1) + s) : l.get(P)),
        g.add("jdusc", "-"),
        g.add("jducp", "-"),
        g.add("jdumd", "-"),
        g.add("jduct", "-"),
    ...
    aM = function() {
        ...
        g = q.get(I);
        o(q, l);
        r.util.Wv("__jda", u(q), g, q.get(aG));
        r.util.Wv("__jdb", f(q), g, q.get(am));
        r.util.Wv("__jdc", q.get(af), g);
        r.util.Wv("__jdv", z(q), g, q.get(V));
        return l.a
    ...
    r.tracker = {
        loading: function(t, s, p, y) {
            var C = p && (p.jdac + "||" + p.jdje + "||" + p.jdsc + "||" + p.jdsr + "||" + p.jdul + "||" + p.jdcs + "||" + az(p.jddt) + "||" + p.jdhn + "||" + p.jdfl + "||" + p.jdos + "||" + p.jdbr + "||" + p.jdbv + "||" + p.jdwb + "||" + p.jdxb + "||" + p.jdyb + "||" + p.jdzb + "||" + p.jdcb + "||" + p.jdusc + "||" + p.jducp + "||" + p.jdumd + "||" + p.jduct + "||" + p.jdlt + "||" + p.jdtad),
            g = r.util.Vv("pin");
            var l = "http://csc.360buy.com/log.ashx?type1=" + az(t) + "&type2=" + az(s) + "&pin=" + az(g[Q] > 0 ? g[0] : "-") + "&uuid=" + p.jduid + "&sid=" + p.jdsid + "&referrer=" + az(p.jdmr || "-") + "&jinfo=" + C + "&data=" + az(aq(y)) + "&callback=?";
            $.getJSON(l, 
            function() {});
            return true
...
function log(c, b) {
    ...
    JA.tracker.aloading(c, b, a && a.slice(2))
